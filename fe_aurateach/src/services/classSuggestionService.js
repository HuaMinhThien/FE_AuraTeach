 ...
 *
 * Luồng:
 *  1. Gia sư gửi xác nhận với courseId cụ thể.
 *  2. Hệ thống kiểm tra lớp còn trống (pending_tutor, chưa có tutor_id).
 *  3. Gán gia sư vào lớp đó ngay lập tức.
 *  4. Nếu lớp đã có gia sư rồi → trả về "Lớp đã có gia sư nhận".
 */
export async function acceptClass(courseId, tutorId) {
  try {
    ...
    // 1. Lấy course được gửi lên
    const courseRes = await fetch(`${API_BASE}/courses?course_id=${courseId}`);
    const courses = await courseRes.json();
    const course = courses[0];
    if (!course) throw new Error('Không tìm thấy lớp học');

    // 2. Kiểm tra lớp đã có gia sư nhận chưa
    if (course.tutor_id) {
      return {
        success: false,
        message: 'Lớp này đã có gia sư nhận rồi! Hẹn bạn ở lớp khác.',
        alreadyAssigned: true,
        classFull: true
      };
    }

    // 3. Kiểm tra lớp còn ở trạng thái chờ tutor không
    if (course.status !== 'pending_tutor') {
      return {
        success: false,
        message: 'Lớp này không còn chờ gia sư nữa.',
        alreadyAssigned: true
      };
    }

    // 4. Kiểm tra gia sư đã nhận lớp này chưa
    const alreadyAccepted = course.tutor_id === actualTutorId;
    if (alreadyAccepted) {
      return {
        success: false,
        message: 'Bạn đã nhận lớp này rồi!',
        alreadyAssigned: true
      };
    }
    ...
    // 7. Cập nhật class_suggestion của tutor → accepted
    const sugRes = await fetch(
      `${API_BASE}/class_suggestions?course_id=${courseId}&tutor_id=${actualTutorId}`
    );
    ...
    // 9. Thông báo cho các tutor khác còn pending → lớp đã có người nhận
    const pendingSuggestionsRes = await fetch(
      `${API_BASE}/class_suggestions?course_id=${courseId}&status=pending`
    );
      ...
      if (sug.tutor_id !== actualTutorId) {
        ...
        if (otherUser) {
          await createNotification({
            ...
            receiver_role: 'tutor',
            type: 'system',
            title: '❌ Lớp đã có gia sư nhận',
            message: `Lớp "${course.title}" đã có gia sư khác nhận trước. Hẹn bạn lần sau!`,
            related_id: courseId,
            related_type: 'class_suggestion'
          });
        }
 ...
 * Mỗi suggestion tương ứng với 1 lớp học — hiển thị tất cả lớp còn đang chờ gia sư.
 */
export async function getTutorSuggestions(tutorId) {
  try {
    ...
    for (const sug of suggestions) {
      ...
      // Bỏ qua lớp đã có gia sư nhận hoặc không còn ở trạng thái chờ
      if (course.tutor_id || course.status !== 'pending_tutor') continue;
